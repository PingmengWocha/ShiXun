...
<head>
<meta charset="UTF-8">
<title>著海学习平台的资源管理</title>
<link rel="stylesheet" href="css/reg.css">
<script type="text/javascript" charset="utf-8">
function goPage(){
	   var v = document.getElementById("selectPage").value;
	   alert("我们将去页面Adownload.jsp?flag="+v);
	   document.open("Adownload.jsp?flag="+v,"_self","");
	}
...
<body>
	<div id="wrap">
		<center>
			...
			<font class=title>著海学习平台的资源</font> <a href="alogin.html">退出登录</a>
			<hr>
			<form>
			...
			<br>
			<form method="post" action="Adownload.jsp?flag=first">
...
for(Ziyuan n:list){%>
					<tr>
						...
						<td><submit>
							<a href="DeleteDownloadServlet?filename=<%=n.getU_name()%>">删除</a></submit></td>
					</tr>
					<% 
}
%>
			...
			<div id="foot">
				<a
					href="Adownload.jsp?flag=<%=SplitPage.FIRSTPAGE %>&item=<%=strItem%>&content=<%=strContent%>">首页</a>
				<a
					href="Adownload.jsp?flag=<%=SplitPage.PREVIOUSEPAGE %>&item=<%=strItem%>&content=<%=strContent%>">上一页</a>
				<a
					href="Adownload.jsp?flag=<%=SplitPage.NEXTPAGE %>&item=<%=strItem%>&content=<%=strContent%>">下一页</a>
				<a
					href="Adownload.jsp?flag=<%=SplitPage.LASTPAGE %>&item=<%=strItem%>&content=<%=strContent%>">尾页</a>
			</div>
