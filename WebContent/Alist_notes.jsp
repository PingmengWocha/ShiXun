...
function goPage(){
	   ...
	   alert("我们将去页面Alist_notes.jsp?flag="+v);
	   document.open("Alist_notes.jsp?flag="+v,"_self","");
	}
...
<div id="wrap">
		<center>
			...
			<font class=title>著海学习平台的留言板</font> <a href="alogin.html">退出登录</a>
			<hr>
			<br>
			<form>
			...
			<br>
			<form method="post" action="Alist_notes.jsp?flag=first">
				<jsp:useBean id="spage" class="sh.util.SplitPage" scope="session"></jsp:useBean>
				<jsp:useBean id="daob" class="sh.dao.impl.NoteDaoSQLServerImpl"
...
for(Note n:list){%>
					<tr>
						...
						<td><submit>
							<a href="AIdmessageServlet?id=<%=n.getM_id()%>">查看详情</a></submit></td>
						<td><submit>
							<a href="DeletemessageServlet?id=<%=n.getM_id()%>">删除留言</a></submit></td>	
					</tr>
					<% 
}
%>
			...
			<div id="foot">
				<a
					href="Alist_notes.jsp?flag=<%=SplitPage.FIRSTPAGE %>&item=<%=strItem%>&content=<%=strContent%>">首页</a>
				<a
					href="Alist_notes.jsp?flag=<%=SplitPage.PREVIOUSEPAGE %>&item=<%=strItem%>&content=<%=strContent%>">上一页</a>
				<a
					href="Alist_notes.jsp?flag=<%=SplitPage.NEXTPAGE %>&item=<%=strItem%>&content=<%=strContent%>">下一页</a>
				<a
					href="Alist_notes.jsp?flag=<%=SplitPage.LASTPAGE %>&item=<%=strItem%>&content=<%=strContent%>">尾页</a>
			</div>
